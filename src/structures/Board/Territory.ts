...
import { Token, BattleResult } from "../../utils/Utils.js";

export class Territory {
    readonly name: string
    readonly neighbors: Territory[]; // instantiated in the continent declarations - will be constant for every game
    ...
    constructor(name: string, neighbors: Territory[], coastal: boolean, port: boolean, castle: boolean) {
        this.name = name;
    ...
    isNeighboring(target: Territory): boolean {
        if (this.neighbors.includes(target) !== target.neighbors.includes(this)) {
            throw new Error("Inconsistent neighbors!");
        }
        return this.neighbors.includes(target);
    }
    ...
    placeCapital() {
        ...
        if (player.capitol) throw new Error("Player already has a seat of power!")
        this.capital = player;
        player.setCapitol(this)
    ...
    changeOwner(newOwner: Player, startOfGame: boolean = false) {
        this.owner = newOwner;
        this.setTroops(startOfGame ? 1 : 0);
    }
